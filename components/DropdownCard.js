...
export default function DropdownCard({ title, desc }) {
  ...
  const toggleTestimonial = () => {
    ...
    if (open) {
      TweenMax.to(dropRef.current, 0.55, { height: '0px', ease: 'Power1.easeOut' })
    } else {
      TweenMax.to(dropRef.current, 0.55, { height: '220px', ease: 'Power1.easeOut' })
      TweenMax.staggerFrom(split.lines, 0.3, { y: 20, opacity: 0, delay: 0.1 }, 0.03)
    }
  }
