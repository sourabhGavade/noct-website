...
export default function Slider({ type, sliderTitle, slides }) {
    ...
    <>
      <div className="slider slider-grid" {...swipeHandlers}>
        <div className="slider-info mobile-full-bleed">
          <div className="h6 mb-3 mb-lg-5 text-uppercase" style={{color: `${type === 'regular' ? 'var(--green)' : 'var(--blue)'}`}}>{sliderTitle}</div>
          {
            type === 'regular' &&
            <>
              <div className="slider-title h5 mb-3">{slide.title}</div>
              <div className="row">
                <div className="col-lg-12 col-xl-9">
                  <p className="slider-desc mb-5">{slide.desc}</p>
                </div>
              </div>
              <Link href="/contact"><a><Button text="Get in Touch" style="normal" /></a></Link>
            </>
          }
          {
            type === 'testimonial' &&
        ...
        { type === 'regular' && <div className="slider-img" style={{ backgroundImage: `url('/images/about/about-causes-${activeSlide}.webp')` }} /> }
        { type === 'testimonial' && <div className="slider-img" style={{ backgroundImage: `url('/images/join-the-network/join-us-testimonial-${activeSlide}.webp')` }} /> }
      </div>
      <style jsx>{`
        ...
        .slider-info {
          ...
          margin-top: 40px;
          margin-bottom: 40px;
          background: ${ type === 'regular' ? 'var(--light-green)' : 'var(--light-blue)'};
        }
