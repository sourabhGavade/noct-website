...
export async function getServerSideProps(context) {
  ...
  const role = context.params.position
...
export default function JoinTheNetworkInner({ position, category, role }) {
  ...
  return (
    <>
      <Head>
        <title>NOCT | Join Us - {position.title}</title>
      </Head>
        ...
        <div className="container">
          <div className="row align-items-center justify-content-between">
            ...
            <div className="order-1 order-lg-2 col-6 col-lg-3 mb-4 mb-lg-0">
              <img src={urlFor(position.illustration).url()} alt="" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container">
          <div className="row justify-content-between">
            <div className="col-lg-8 mb-5 mb-lg-0">
              <div className="h2 mb-4">{position.statsTitle}</div>
              <BlockContent blocks={position.statsDescription} />
            </div>
      ...
      {
        position.imageCarousel && position.imageCarousel.length > 0 &&
        ...
        <div className="container">
          <div className="row justify-content-between">
            <div className="col-lg-4">
              <div className={`h2 ${position.process.length > 5 ? 'sticky' : ''}`}>Here's how the process works</div>
            </div>
            <div className="col-lg-7">
              {
                role === "front-end-developer" ? (
                  <VerticalTimeline items={position.process?.map((item, index) => {
                    if (index === 2) {
                      return {
                        image: item.image,
                        title: "Assignment",
                        description: "You will be assigned a task to showcase your coding skills in frontend technologies (HTML, CSS, JavaScript, React)."
                      }
                    } else {
                      return item
                    }
                  ...
                  <VerticalTimeline items={position.process} />
                )
              }
            </div>
          </div>
      ...
      <section className="padded-section bg-grey">
        <div className="container">
          <div className="row align-items-center justify-content-between">
            ...
            <div className="col-lg-6">
              <div className="h3 mb-2">Ready to be part of our team?</div>
