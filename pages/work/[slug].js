...
export default function ProjectPage({ project, allProjects }) {
  ...
  useEffect(() => {
    if (isAuthenticated && prevImageRef.current && nextImageRef.current) {
  ...
  }, [isAuthenticated, containerOffset, project.bgColor]);
  ...
  const handlePasswordClose = () => {
    // Redirect to work page if user closes the password modal
    router.push("/work");
  };
    ...
    <>
      <Head>
        ...
        <link
          rel="preload"
          as="image"
          href={urlFor(project.mainImageDesktop).url()}
        />
        <link
          rel="preload"
          as="image"
          href={urlFor(project.mainImageMobile).url()}
        />
      </Head>
      ...
      {/* Show project content only if authenticated or not password protected */}
      {isAuthenticated && (
        <article className="project-page">
          ...
          {project.projectSections.map((section, index) => (
            <div key={section._key}>
              ...
              {section._type === "singleMedia" && (
                <SingleMedia data={section} />
              )}
